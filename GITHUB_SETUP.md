...
A `.gitignore` already ships at the repo root. It excludes Python caches,
the user registry `*.db`, generated example outputs, `node_modules/`, built
frontend `dist/`, saved canvases, and `.env` files. Note the generated-output
patterns are scoped (e.g. `examples/*.json`) so the web app's `package.json`
and `tsconfig.json` stay tracked.
...
```bash
git add .
git commit -m "Initial commit: Anvil framework v1.3.0"
```
