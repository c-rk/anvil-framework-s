 ...
 */
/** A consumer that isn't a relation (e.g. an arithmetic block input port). */
export interface ExtraConsumer {
  nodeId: string;
  /** Variable name this port consumes. */
  name: string;
  /** Target handle id (e.g. "in:a"). */
  handle: string;
}
...
export function computeWires(
  ...
  for (const c of extraConsumers) {
    if (c.name.trim()) push(consumers, c.name.trim(), { nodeId: c.nodeId, handle: c.handle });
  }
