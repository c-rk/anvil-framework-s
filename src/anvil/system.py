...
# ============================================================
# Parallel execution backend (M2)
# ============================================================
#
# The sweep / DOE executor is pluggable. The default backend is "process"
# (ProcessPoolExecutor) for true CPU scaling across cores; it falls back to
# "thread" automatically when the System cannot be pickled (e.g. a relation's
# func is a closure or a registry-loaded function whose qualified name does not
# resolve in the worker). "serial" forces single-process execution.

_DEFAULT_BACKEND = "process"


def set_backend(backend):
    """
    Set the default parallel backend for sweeps / DOE.

    Parameters
    ----------
    backend : {"process", "thread", "serial"}
        "process" (default) -- ProcessPoolExecutor, true CPU scaling.
        "thread"            -- ThreadPoolExecutor, best for numpy/scipy that
                               releases the GIL; always picklable-safe.
        "serial"            -- no concurrency (ignores worker count).
    """
    ...
    if backend not in ("process", "thread", "serial"):
        raise ValueError(
            ...
            f"Choose from 'process', 'thread', 'serial'."
        )
...
def get_backend():
    """Return the current default parallel backend."""
    return _DEFAULT_BACKEND


def _is_picklable(obj):
    """True if obj can be pickled (required for ProcessPoolExecutor)."""
    import pickle
    try:
        pickle.dumps(obj)
        return True
...
# Top-level worker so it is itself picklable under the "spawn" start method
# used on Windows. Receives a fully-built System copy plus solve kwargs.
def _solve_worker(payload):
    sys_copy, solve_kwargs = payload
    return sys_copy.solve(**solve_kwargs)


def _run_parallel(systems, solve_kwargs, n_workers, backend=None,
                  skip_errors=False, labels=None):
    """
    Evaluate a list of (already-configured) System copies concurrently.

    Returns a list of results (or None on failure when skip_errors=True),
    in the SAME order as ``systems``.

    backend : None uses the module default (set_backend). "process" is
    auto-downgraded to "thread" with a warning if any system is unpicklable.
    """
    ...
    if n_workers <= 1 or backend == "serial" or n <= 1:
        backend = "serial"

    # Process backend requires picklable payloads; verify and downgrade if not.
    if backend == "process":
        probe = systems[0]
        if not (_is_picklable(probe) and _is_picklable(solve_kwargs)
                and _is_picklable(_solve_worker)):
            _warnings_mod.warn(
                "Parallel backend 'process' requested but the System is not "
                "picklable (likely a relation wrapping a closure / local or "
                "registry-loaded function). Falling back to 'thread'.",
                stacklevel=3,
            )
            backend = "thread"
    ...
    if backend == "serial":
        for i, sys_copy in enumerate(systems):
            try:
                results_map[i] = sys_copy.solve(**solve_kwargs)
            except Exception as e:
                results_map[i] = _handle_worker_error(e, labels[i], skip_errors)
        return [results_map[i] for i in range(n)]

    if backend == "process":
        Executor = ProcessPoolExecutor
        submit_arg = [(s, solve_kwargs) for s in systems]
        fn = _solve_worker
    else:  # thread
        Executor = ThreadPoolExecutor
        submit_arg = systems
        fn = lambda s: s.solve(**solve_kwargs)

    try:
        with Executor(max_workers=n_workers) as pool:
            futures = {pool.submit(fn, submit_arg[i]): i for i in range(n)}
        ...
        # ProcessPool can fail to even start workers (e.g. spawn / pickling at
        # submit time on Windows). Fall back to threads and retry once.
        if backend == "process":
            _warnings_mod.warn(
                f"Process pool failed to run ({e}); retrying with thread pool.",
                stacklevel=3,
            )
            with ThreadPoolExecutor(max_workers=n_workers) as pool:
                futures = {pool.submit(lambda s: s.solve(**solve_kwargs),
                                       systems[i]): i for i in range(n)}
                for fut in as_completed(futures):
                    i = futures[fut]
                    try:
                        results_map[i] = fut.result()
                    except Exception as e2:
                        results_map[i] = _handle_worker_error(
                            e2, labels[i], skip_errors)
        else:
            raise
...
def _handle_worker_error(exc, label, skip_errors):
    """Either swallow (returning None) or re-raise a worker exception."""
    if skip_errors:
        _warnings_mod.warn(
            f"Parallel evaluation point {label} failed: {exc}", stacklevel=4
        )
        return None
    raise RuntimeError(f"Parallel evaluation failed at {label}.\n  {exc}") from exc


class Result:
    """Solve results with auto-formatting and data export."""

    def __init__(self, data, system_name="", inputs=None, method=""):
        self._data = data
        self._system_name = system_name
        self._inputs = inputs or set()
        self.method = method  # solver method actually used (forward/gauss_seidel/newton)
...
class System:
    ...
    def copy(self):
        ...
        # Clone Relation wrappers so per-instance caches (_qty_compatible,
        # discovered _outputs) are isolated. The underlying func is shared
        # (it is stateless), but a copy must be safe for independent solves.
        import copy as _copy
        cloned = []
        for rel in self._relations:
            r2 = _copy.copy(rel)
            r2._qty_compatible = None
            r2._outputs = list(getattr(rel, "_outputs", []))
            cloned.append(r2)
        new._relations = cloned
    ...
    def solve(self, method=None, max_iter=100, rtol=1e-6, atol=1e-10,
              relaxation=1.0, monitor=False, verbose=False, on_iter=None):
        """
        Solve the system.
        ...
        on_iter : callable, optional
            Per-iteration callback for iterative solvers (gauss_seidel). Called
            once per iteration as ``on_iter({"iter": i, "residual": max_change,
            "variables": {...}})``. Default None disables it and changes nothing
            (fully backward compatible). Forward / direct solves never iterate,
            so the callback simply is not called for them.
        """
        ...
        for key, val in ws.items():
            if isinstance(val, (dict, list, str)):
                data[key] = val  # complex / string output types stored as-is
        ...
        return Result(data, self.name, inputs=set(self._quantities.keys()), method=method)
    ...
    def _iterate(self, ws, max_iter, rtol, relaxation, monitor, verbose, t0,
        ...
        for iteration in range(max_iter):
            ...
            # Optional live-iteration callback (backward compatible: default None).
            # Emitted before the monitor/verbose printing and convergence check so
            # consumers (e.g. a streaming WebSocket) see every iteration in order.
    ...
    def sweep(self, param_name, values, skip_errors=False, parallel=1,
              warm_start=False, backend=None, **solve_kwargs):
        """
        ...
        parallel : int
            ...
            Defaults to a process-based pool (true CPU scaling). If the System
            cannot be pickled (e.g. a relation wraps a closure / local function),
            it transparently falls back to a thread pool with a warning.
            See ``set_backend()`` to force ``"process"``, ``"thread"`` or ``"serial"``.
        ...
        **solve_kwargs
        ...
        if parallel > 1:
            return self._sweep_parallel(
                ...
                skip_errors, parallel, solve_kwargs, backend
            )
    ...
    def _sweep_parallel(self, param_name, values, orig, scale, param_unit,
                        skip_errors, n_workers, solve_kwargs, backend=None):
        """
        Run sweep concurrently using a pluggable executor.

        Builds one independent System copy per sweep point (System.copy()
        deep-clones relations, so copies are safe to evaluate concurrently or
        in separate processes), then dispatches via ``_run_parallel``. The
        default backend is process-based for true CPU scaling, with automatic,
        warned fallback to threads when the System is not picklable.
        """
        systems = []
        for v in values:
            sys_copy = self.copy()
            sys_copy._quantities[param_name] = Quantity._raw(
                ...
                unit_hint=orig._unit_hint)
            sys_copy._validated = False
            systems.append(sys_copy)

        labels = [f"{param_name}={float(v):.4g}" for v in values]
        ordered_results = _run_parallel(
            systems, solve_kwargs, n_workers, backend=backend,
