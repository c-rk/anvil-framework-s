...
def _rebuild_namespaces():
    ...
    for record in store.get_all():
        try:
            ...
            failures.append((record.get("name", "?"), f"{type(e).__name__}: {e}"))
            continue
    ...
    if failures:
        import warnings
        detail = "; ".join(f"{n} ({err})" for n, err in failures)
        warnings.warn(
            f"{len(failures)} RSQ(s) failed to load and were skipped: {detail}",
