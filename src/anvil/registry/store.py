...
def _like_escape(s):
    """Escape LIKE wildcards so '_' and '%' match literally (paired with ESCAPE '\\').

    RSQ names routinely contain '_' (e.g. 'isentropic_ratios'); without this a
    search/domain filter would treat it as a single-char wildcard.
    """
...
class Store:
    ...
    def get_all(self, rsq_type=None, domain=None, origin=None, tag=None):
        ...
        if domain:
            conditions.append("(rsq.domain = ? OR rsq.domain LIKE ? ESCAPE '\\')")
            params.extend([domain, f"{_like_escape(domain)}.%"])
        if origin:
    ...
    def search(self, keyword):
        ...
        rows = conn.execute("""
            ...
            WHERE rsq.name LIKE ? ESCAPE '\\' OR rsq.description LIKE ? ESCAPE '\\'
                OR rsq.domain LIKE ? ESCAPE '\\' OR tags.tag LIKE ? ESCAPE '\\'
