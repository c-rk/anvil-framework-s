...
class Quantity:
    ...
    def _linear_hint(self):
        """Unit hint safe for add/sub results: offset units (degC, degF) are
        dropped so sums/differences display in the absolute unit (K) instead
        of re-applying the offset to a value that no longer carries it."""
        if self._unit_hint and _db.get_offset(self._unit_hint) != 0.0:
            return ""
        return self._unit_hint
    ...
    def __eq__(self, other):
        if isinstance(other, Quantity):
            if self._dim != other._dim:
                return False
            if self._si_value is None or other._si_value is None:
                return self._si_value is None and other._si_value is None
    ...
    def _repr_latex_(self):
        ...
        if u:
            import re as _re
            u_latex = _re.sub(r"\^(-?\d+(?:\.\d+)?)", r"^{\1}", u)
            return f"${v}\\;\\mathrm{{{u_latex}}}$"
        return f"${v}$"
