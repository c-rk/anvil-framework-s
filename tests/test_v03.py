...
@check("Q stores SI, displays in user unit")
def _():
    q = Q(1, "kPa")
    close(q.si, 1000.0)
    close(q.value, 1.0)
...
@check("set with bare number keeps unit")
def _():
    s = System("t")
    s.add("P0", 6.9e6, "Pa")
    s.set(P0=8e6)
...
@check("set with Q overrides unit")
def _():
    s = System("t")
    s.add("P0", 6.9e6, "Pa")
    s.set(P0=Q(1000, "psi"))
...
@check("set unknown variable raises KeyError")
def _():
...
@check("composition via as_relation")
def _():
...
@check("filter by type and domain")
def _():
    with tempfile.TemporaryDirectory() as td:
        st = Store(os.path.join(td, "test.db"))
...
@check("flat and hierarchical point to same object")
def _():
    ns = Namespace("test")
    obj = object()
...
@check("missing attribute gives helpful error")
def _():
    ns = Namespace("test")
...
@check("tab completion via __dir__")
def _():
    ns = Namespace("test")
...
@check("load a Relation from source")
def _():
    record = {
        "name": "test_double",
        "type": "R",
...
@check("load a Quantity from source")
def _():
    record = {
        "name": "test_g",
        "type": "Q",
