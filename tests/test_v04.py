...
@check("cross-ref force -> N, pressure -> Pa/psi")
def _():
...
@check("custom unit auto-creates and propagates")
def _():
    from anvil import Q
    a = Q(5, "blips") * Q(3, "s")
...
@check("arithmetic -> correct units")
def _():
...
@check("set keeps unit, Q overrides")
def _():
...
@check("as_relation doesnt mutate original")
def _():
...
@check("forward + sweep + name mapping")
def _():
...
@check("Result.to_dict and SweepResult.to_dict")
def _():
    def sq(x): return {"y": x**2}
...
@check("heat exchanger converges + energy balance")
def _():
...
@check("python adapter + in system")
def _():
...
@check("diagnose catches missing inputs")
def _():
...
@check("diagnose reports coupled variables")
def _():
...
@check("diagnose reports bounds violations")
def _():
...
@check("structures: cantilever beam")
def _():
...
@check("root finding + nonlinear + ODE + minimize")
def _():
