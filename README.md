...
- **101 units** with automatic dimension tracking through all arithmetic; offset temperature units (`degC`, `degF`)
- **101 registered RSQs** across 20+ domains: aero, propulsion, structures, heat transfer, orbital, thermo, controls, materials, mission, combustion, signal processing, decomposition, UQ, constants
- **Web workbench**: calculator + node-graph canvas UI (`anvil_web`) served by a FastAPI backend (`anvil_server`) — RSQ spotlight search, live solver residuals over WebSocket, canvases saved as runnable Python scripts, keyboard shortcuts, offline docs at `/wiki`
...
`anvil_server/README.md` for tiers, sandboxing, and endpoint reference.
